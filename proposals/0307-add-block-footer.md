...
from a regular entry batch. A block meta chunk has the following versioned header:
...
`block_meta_flag: u64`: will always be zero. The first 8 bytes of an entry
batch are always a positive number (the number of entries in the batch), so
this flag allows parsers to differentiate the block meta from a normal entry
batch.
...
- `variant: u8` is a positive integer which identifies the structure of the block
meta payload. For example, the block footer will be identified by `variant=1`.
New metadata may be added without changing the footer by adding a new variant
which corresponds to a differently specified payload.

- `length: u16` is the length of the block meta payload in bytes (i.e.
not including the `block_meta_flag`, `version`, `variant`, and `length` fields).
Though not necessary, this will make it easier for block parsers to
...
- `footer_version: u16` is a positive integer which changes anytime a change is made to
...
Any other fields that are deemed necessary in the future may be added in one of two ways.
